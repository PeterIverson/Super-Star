...
class BottleneckBlock(nn.Module):
    ...
    def init_k(self, x):
        ...
        self.k_sum = self.k  # sum of all input vectors mapped to the same code within a batch.
        self.k_elem = t.ones(k_bins, device=self.k.device)  # count how many times each code appears.
    ...
    # x is the input, x_l is the code index obtained after feeding x into the codebook
    def update_k(self, x, x_l):
        ...
        with t.no_grad():
            ...
            # usage records the code indices used in this epoch, which are the ones to be updated
            usage = (self.k_elem.view(k_bins, 1) >= self.threshold).float()  # k_bins, 1
            '''
                Update method:
                    1. If a code has been used, compute the new code as the average of the new inputs mapped to it.
                    2. If a code has not been used, regenerate a new random one for it.
            '''
    ...
    # the returned x_l is the index of the nearest code found in the codebook for input x
    def quantise(self, x):
    ...
    # the returned value is the code vector corresponding to x_l
