...
class CrossCondGPT2_2part_Audio_Cross(nn.Module):
    ...
    def sample(self, xs, cond, shift=None):
        ...
        for k in range(cond.size(1)):
            ...
            cond_input = cond[:, :block_size] if k < block_size else cond[:, k-(block_shift+(k-block_size-1)%(block_size-block_shift+1))+1:k+1] 
            ...
            # set the top-x value during inference; x > 1 increases generation diversity, x = 1 is a deterministic model
            _, ix_body = torch.topk(probs_body, k=5, dim=-1)
            _, ix_hands = torch.topk(probs_hands, k=5, dim=-1)
            
            # manually wrote a probability selector to choose the next token based on the probability distribution
    ...
    def forward(self, idxs, cond, targets=None):
        ...
        logits_body, logits_hands, loss_body, loss_hands, metrics = self.gpt_head(feat, cond, targets)
...
# cross-attention layer that accepts conditional input
class CrossConditionalCrossAttention(nn.Module):
    """
    A multi-head masked cross-attention layer with a projection at the end.
    This layer accepts conditional input for cross-attention.
    """
    ...
    def forward(self, x, cond_emb):
        B, T, C = x.size()
        Bc, Tc, Cc = cond_emb.size()
        
        # calculate query from input x
        q = self.query(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        
        # calculate key and value from condition embedding (use cond_emb's own time dim)
        k = self.key(cond_emb).view(Bc, Tc, self.n_head, Cc // self.n_head).transpose(1, 2)  # (B, nh, Tc, hs)
        v = self.value(cond_emb).view(Bc, Tc, self.n_head, Cc // self.n_head).transpose(1, 2)  # (B, nh, Tc, hs)
        
        t = T // self.n_modality
        
        # causal cross-attention: (B, nh, T, hs) x (B, nh, hs, Tc) -> (B, nh, T, Tc)
...
# masked self-attention layer that can only see previous information
...
class CrossCondBlock(nn.Module):
    """ Transformer block with cross-attention for conditional input """
    ...
    def __init__(self, config):
        ...
        self.ln1 = nn.LayerNorm(config.n_embd)
        self.ln2 = nn.LayerNorm(config.n_embd)
        self.attn1 = CausalCrossConditionalSelfAttention(config)
        self.attn2 = CrossConditionalCrossAttention(config)
        self.mlp = nn.Sequential(
            nn.Linear(config.n_embd, 4 * config.n_embd),
            nn.GELU(),
            nn.Linear(4 * config.n_embd, config.n_embd),
    ...
    def forward(self, x, cond_emb):
        x = x + self.attn1(self.ln0(x))
        x = x + self.attn2(self.ln1(x), cond_emb)
        x = x + self.mlp(self.ln2(x))
        return x
...
class CrossCondGPTBase(nn.Module):
    ...
    def __init__(self, config):
        ...
        # for 1D integer values, convert via nn.Embedding; for high-dimensional float values, convert via nn.Linear
        self.n_modality = config.n_modality
        ...
        # transformer - using cross-attention blocks for conditional input
        self.blocks = nn.Sequential(*[CrossCondBlock(config) for _ in range(config.n_layer)])
    ...
    def forward(self, idx_body, idx_hands, cond):
        ...
        token_embeddings = torch.cat([self.cond_emb(cond)[:, :t, :], token_embeddings_body, token_embeddings_hands], dim=1)

        # equivalent to learning 4 position embeddings, but concatenated together
        ...
        x = self.drop(token_embeddings + position_embeddings) # torch.Size([256, 42, 768])

        # use the conditional embedding as input to cross-attention
        cond_emb = self.cond_emb(cond) # cond_emb.shape = torch.Size([256, 14, 768])
        # add positional encoding to the conditional embedding (reusing the audio segment's positional encoding) so that the K/V ends of cross-attention perceive the time order
        cond_pos = self.pos_emb[:, :cond_emb.size(1), :]
        cond_emb = self.drop(cond_emb + cond_pos)
        # expand cond_emb to match the number of time steps of x (n_modality copies, each responsible for one modality's cross-attention)
        repeat_factor = self.n_modality
        cond_emb_expanded = cond_emb.repeat(1, repeat_factor, 1)  # cond_emb_expanded.shape = torch.Size([256, 42, 768])

        # use cross-attention blocks, passing in the conditional embedding
        for block in self.blocks:
            x = block(x, cond_emb_expanded)
...
class CrossCondGPTHead(nn.Module):
    ...
    def forward(self, x, cond, targets=None):
        # expand cond_emb to match the number of time steps of x
        cond_emb = self.cond_emb(cond)
        cond_pos = self.pos_emb[:, :cond_emb.size(1), :]
        cond_emb = self.drop(cond_emb + cond_pos)
        repeat_factor = x.size(1) // cond_emb.size(1)
        cond_emb_expanded = cond_emb.repeat(1, repeat_factor, 1)
        ...
        N, T, C = x.size()
        t = T // self.n_modality
        # here we discard the [:, 0:t] part related to the audio information
