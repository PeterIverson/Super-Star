...
class Fine_GPT2_2part_Audio_Cross(nn.Module):
    ...
    def __init__(self, config):
        ...
        # transformer
        self.blocks = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=config.n_embd,
                                nhead=config.n_head,
    ...
    def forward(self, idxs, cond, targets=None):
        ...
        tgt = self.drop(token_embeddings + position_embeddings)

        # add positional encoding to the conditional input
        cond_embeddings = self.cond_emb(cond)
        cond_position_embeddings = self.pos_emb[:, :t, :]
        cond_input = self.drop(cond_embeddings + cond_position_embeddings)

        # expand cond_emb to match the number of time steps of x
        cond_input_expanded = cond_input.repeat(1, tgt.size(1) // cond_input.size(1), 1)

        # use TransformerDecoder, with the conditional input as memory and token_embeddings as tgt
        x = self.blocks(tgt=tgt, memory=cond_input_expanded)
        # x = self.ln_f(x)
