...
class SepVQVAE_DDP_body_hands(nn.Module):
    ...
    def decode(self, zs, start_level=0, end_level=None, bs_chunks=1, insert_pos=None, stride=1, filter_name='savgol', filter_pos=None, skip_botleneck=False, only_base_layer=False):
        ...
        x_body = self.vqvae_body.decode(z_body, insert_pos=insert_pos, stride=stride, filter_name=filter_name, filter_pos=filter_pos, skip_botleneck=skip_botleneck, only_base_layer=only_base_layer)
        x_hands = self.vqvae_hands.decode(z_hands, insert_pos=insert_pos, stride=stride, filter_name=filter_name, filter_pos=filter_pos, skip_botleneck=skip_botleneck, only_base_layer=only_base_layer)
