...
class ResidualVQ(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        num_quantizers, # number of codebooks in the rq
        codebook_dim = None, # codebook dimension; we assert codebook_dim == dim, so codebook_dim need not be specified
        shared_codebook = False, # all layers share the same codebook
        heads = 1, # heads == 1
    ...
    # input the indices of each rq layer, output the code obtained for each layer
    ...
    # sum the codes obtained for each layer in the previous function to get the final code
    ...
    def forward(
        ...
        x,
        indices = None,
        return_all_codes = False, # if set to true, the return value includes an extra all_codes, containing the quantized values of each vq layer through its corresponding codebook
        ...
        # all_losses: (quantizer, batch) in the Image Generation RVQ paper, the commitment loss for each rq layer is averaged
