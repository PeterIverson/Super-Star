...
# data shorter than window_size will be discarded
class Big_MASeq_Train_h5(Dataset):
    def __init__(self, data_dir, body_motions_name, body_motions_len, window_size, stride, ds_rate, return_start_frame_num=False):
        def recursively_load(h5obj):
            if isinstance(h5obj, h5py.Dataset):
                return h5obj[()]  # load as a numpy array or scalar
            elif isinstance(h5obj, h5py.Group):
                return {key: recursively_load(h5obj[key]) for key in h5obj.keys()}
        # self.body_motions = np.float32(body_motions)
        self.data_dir = data_dir
        # self.motion_h5_dir = os.path.join(data_dir, 'h5')
        # self.audio_h5_dir = os.path.join(data_dir, 'h5_audio')

        with h5py.File(os.path.join(data_dir, 'merged.h5'), 'r') as f:
            self.motion_h5_dic = {key: recursively_load(f[key]) for key in f.keys()}

        with h5py.File(os.path.join(data_dir, 'merged_audio.h5'), 'r') as f:
            self.audio_h5_dic = {key: recursively_load(f[key]) for key in f.keys()}
        ...
        self.block_index2motion = [] # record which motion file each sliding-window block belongs to
        self.start_block_index = np.zeros(len(self.body_motions_name), dtype = int) # record the first block index (after sliding window) for each motion file
        block_length = 0
        ...
        self.length = block_length # total number of blocks after sliding window
        self.return_start_frame_num = return_start_frame_num
    ...
    def __getitem__(self, index):
        ...
        file_name = self.body_motions_name[bvh_index]
        # motion_file_path = os.path.join(self.motion_h5_dir, file_name)
        # if os.path.exists(motion_file_path):
        #     with h5py.File(motion_file_path, 'r') as f:
        #         motion_block = f['value'][motion_start_frame_num: motion_start_frame_num+self.window_size]
        ...
        motion_block = self.motion_h5_dic[file_name][motion_start_frame_num: motion_start_frame_num+self.window_size]
        ...
        #     print(audio_file_path)
        #     raise(NotImplementedError)
        audio_block = self.audio_h5_dic[file_name][audio_start_frame_num: audio_start_frame_num+self.window_size_audio]

        if self.return_start_frame_num:
            return np.array(motion_block, dtype = np.float32), np.array(audio_block, dtype = np.float32), motion_start_frame_num
        else:
            return np.array(motion_block, dtype=np.float32), np.array(audio_block, dtype=np.float32)