...
class SMPLX2020Render():

    def __init__(self, SMPLX2020_MODEL_PATH):
        if torch.cuda.is_available():
            self.device = torch.device("cuda")
        else:
            self.device = torch.device("cpu")

        self.smplx_2020 = smplx.create(SMPLX2020_MODEL_PATH,
                                       model_type='smplx',
                                       gender='NEUTRAL_2020',
    ...
    def init_renderer(self, res, smpl_param, is_headroot=False):
        ...
        poses = torch.from_numpy(smpl_param['pred_pose']).to(self.device)
        trans = torch.from_numpy(smpl_param['pred_trans']).to(self.device)
        exps = torch.from_numpy(smpl_param['pred_exps']).to(self.device)
        pred_betas = np.tile(smpl_param['pred_shape'].reshape(1, 300).astype(np.float32), (poses.shape[0], 1))
        pred_betas = torch.from_numpy(pred_betas).to(self.device)
        pred_root = torch.tensor(smpl_param['pred_root'].reshape(-1, 3).astype(np.float32), device=self.device)

        smpl_output = self.smplx_2020(
            betas=pred_betas[:],
            transl=trans[:],
            expression=exps[:],
        ...
        self.renderer = Renderer(vertices=self.vertices,
                                 focal_length=self.focal_length,
                                 img_res=(res[1], res[0]),
                                 faces=self.smplx_2020.faces)

    def render(self, index):
        renderImg = self.renderer(self.vertices[index, ...],
                                  self.pred_camera_t)
        # renderImg = rotate(renderImg, 180)
