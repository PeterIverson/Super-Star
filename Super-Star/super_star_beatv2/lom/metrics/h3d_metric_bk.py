...
class H3DMetrics(Metric):
    ...
    @torch.no_grad()
    def update(self, rec_pose: Tensor, tar_pose: Tensor, lengths: Tensor = None):
        ...
        for i in range(bs):
            ...
            # Flatten time and joints dimensions
            T, J, C = rec_joints[i].shape
            length = torch.tensor(lengths[i], device=rec_joints.device)

            rec_flat = rec_joints[i, :length].reshape(length, J, C)
            tar_flat = tar_joints[i, :length].reshape(length, J, C)

            # Calculate metrics
            mpjpe_val = self.calc_mpjpe(rec_flat * 1000, tar_flat * 1000).mean().item()
            self.mpjpe += mpjpe_val
            
            pampjpe_val = self.calc_pampjpe(rec_flat * 1000, tar_flat * 1000).mean().item()
            self.pampjpe += pampjpe_val
            
            accel_val = self.calc_accel(rec_flat * 1000, tar_flat * 1000).mean().item()
    ...
    def compute(self, sanity_flag=False):
        ...
        metrics = {
            "mpjpe": self.mpjpe / count,
            "pampjpe": self.pampjpe / count,
            "accel": self.accel / count
        }
        return metrics