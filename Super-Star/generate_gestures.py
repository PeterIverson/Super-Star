...
def generate(data_dir, audio_name, audio_path, gpt_path, rqvae_path, use_gpu, init_body_pose_code, init_hands_pose_code, local_rank, only_base_layer=False):
    ...
    # decide how many GPT layers to load based on only_base_layer
    if only_base_layer:
        # only load BaseGPT (layer 0)
        gpt_path_to_load = [gpt_path[0]]
    else:
        gpt_path_to_load = gpt_path

    model_gpt = []
    for i in range(len(gpt_path_to_load)):
        gpt_checkpoint = torch.load(gpt_path_to_load[i], map_location='cpu')
    ...
    with torch.no_grad():
        ...
        if only_base_layer:
            # ====== Use only the first-layer token of BaseGPT, skipping the FinetuneGPT residual layers ======
            print("Base Model only ...")
            # zs[0][0]: [1, len] (body), zs[1][0]: [1, len] (hands)
            # construct a code containing only the first layer, shape: [1, 1, len] (only 1 RQ layer)
            body_code_all = zs[0][0].unsqueeze(0)   # [1, 1, len]
            hands_code_all = zs[1][0].unsqueeze(0)   # [1, 1, len]

            tuple_code = ([body_code_all.permute(1,0,2)], [hands_code_all.permute(1,0,2)])

            t_mid5 = time.perf_counter()
            print(f"Time layer1: {t_mid2 - t_mid1}")

            # only_base_layer=True: decode using only the first RQVAE codebook, without accumulating residual layers
            # generated_result_rq_s1 = model_vqvae.module.decode(tuple_code, stride=1, only_base_layer=True)
            generated_result_rq_s1_filter = model_vqvae.module.decode(tuple_code, stride=1, filter_name='savgol', filter_pos='rq_code', only_base_layer=True)
        else:
            # ====== Offline: BaseGPT + FinetuneGPT multi-layer residuals ======
            print("Base Model + Finetuning Layer ...")
            # body_0: [bs, 15], audio_feats_for_inference:[bs, 15, 437]  bs = len - 15 + 1
            ...
            body_code_all = torch.cat((body_code_all[:,0:1,:], x_out_last_frame_body), dim=2) # [4, 1, 15 + bs-1]
            ...
            t_mid5 = time.perf_counter()
            print(f"Time layer1: {t_mid2 - t_mid1} layer24: {t_mid4 - t_mid3}")
            print(f"Time concat: {t_mid5 - t_mid4}")
        ...
        # save_name = ['_original_motion']
        save_name = ['']

    t_end = time.perf_counter()
    print(f"Total Time: {t_end - t_start} ")

    # rq_config.data.dir = "./Data/JIYI_processed"
    ...
    for i in range(len(need_visulize_motion)):
        ...
        if args.eval_dir and not os.path.exists(args.eval_dir):
            os.mkdir(args.eval_dir)
        eval_dir = args.eval_dir if args.eval_dir else data_dir
        to_bvh(data, gpt_config, os.path.join(eval_dir, audio_name+save_name[i]+'.bvh'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_dir', type=str, required=False, default = '')
    ...
    parser.add_argument('--model_path_1', type=str, required=False, default = '')
    parser.add_argument('--model_path_2', type=str, required=False, default = '')
    parser.add_argument('--model_path_3', type=str, required=False, default = '')
    ...
    parser.add_argument('--only_base_layer', action='store_true', default=False,
                        help='use only the first-layer BaseGPT token for decoding, skipping the FinetuneGPT residual layers')
    ...
    # build the gpt_path list
    if args.only_base_layer:
        gpt_path_list = [args.model_path_0]
    else:
        gpt_path_list = [args.model_path_0, args.model_path_1, args.model_path_2, args.model_path_3]
    ...
    generate(data_dir=new_save_dir, audio_name=audio_name, audio_path=args.audio_path, 
             gpt_path=gpt_path_list, rqvae_path=args.rqvae_path, 
             ...
             only_base_layer=args.only_base_layer)
