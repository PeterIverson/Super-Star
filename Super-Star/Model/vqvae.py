...
class VQVAE(nn.Module):
    def __init__(self, hps, input_dim):
        super().__init__()
        self.hps = hps  # model structure parameters.

        input_shape = (hps.sample_length, input_dim)  # 4s(240 frames) x 3J.
        levels = hps.levels
        downs_t = hps.downs_t
    ...
    def _decode(self, zs, start_level=0, end_level=None, insert_pos=None, stride=1, filter_name='savgol', filter_pos=None, skip_botleneck=False, only_base_layer=False):
        # Decode
        ...
        if not skip_botleneck:
            ...
            if self.bottleneck_type == 'rqvae':
                if only_base_layer:
                    # only use the first RQ codebook for decoding, ignoring the residual layers produced by FinetuneGPT
                    # decode_for_1_layer input [1, len], returns [1, len, dim] (3D), just permute directly
                    xs_quantised = self.bottleneck.decode_for_1_layer(zs[0][0])
                    xs_quantised = [xs_quantised.permute(0, 2, 1)]
                else:
                    # decode input [4, len], returns [len, dim] (2D), needs unsqueeze(0)
    ...
    def decode(self, zs, start_level=0, end_level=None, bs_chunks=1, insert_pos=None, stride=1, filter_name='savgol', filter_pos=None, skip_botleneck=False, only_base_layer=False):
        # print(zs.shape)
        z_chunks = [t.chunk(z, bs_chunks, dim=0) for z in zs]
        x_outs = []
        for i in range(bs_chunks):
            zs_i = [z_chunk[i] for z_chunk in z_chunks]
            x_out = self._decode(zs_i, start_level=start_level, end_level=end_level, insert_pos=insert_pos, stride=stride, filter_name=filter_name, filter_pos=filter_pos, skip_botleneck=skip_botleneck, only_base_layer=only_base_layer)
            x_outs.append(x_out)
    ...
    def forward(self, x):
        ...
        for level in reversed(range(self.levels)):
            ...
            regularization += t.mean((x_out[:, 1:] - x_out[:, :-1]) ** 2) # constrain the velocity from being too large
