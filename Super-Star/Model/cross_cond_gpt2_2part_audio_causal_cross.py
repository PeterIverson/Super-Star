...
class CrossCondGPT2_2part_Audio_Causal_Cross(nn.Module):
...
# masked cross-attention layer that accepts conditional input
class CausalCrossConditionalCrossAttention(nn.Module):
    """
    ...
    def forward(self, x, cond_emb):
        ...
        # calculate key and value from condition embedding
        k = self.key(cond_emb).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        v = self.value(cond_emb).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        
        t = T // self.n_modality
        
        # causal cross-attention: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
...
class CrossCondBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn2 = CausalCrossConditionalCrossAttention(config)
        self.mlp = nn.Sequential(
            nn.Linear(config.n_embd, 4 * config.n_embd),
            nn.GELU(),
            nn.Linear(4 * config.n_embd, config.n_embd),
