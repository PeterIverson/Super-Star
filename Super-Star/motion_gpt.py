    ...
    def train(self):
        ...
        for epoch in range(1, self.config.epochs+1):
            ...
            for batch in self.train_loader:
                ...
                audio_feats = audio_feats.to(self.device) # torch.Size([256, 15, 437])  torch.Size([22, 15, 437])

                self.optimizer.zero_grad()

                if self.config.structure_gpt.name in ['CrossCondGPT2', 'CrossCondGPT2_2part', 'CrossCondGPT2_2part_Audio_Causal_Cross', 'CrossCondGPT2_2part_Audio_Cross']:
                    with torch.no_grad():
                ...
                elif self.config.structure_gpt.name in ['Fine_GPT2', 'Fine_GPT2_2part', 'Fine_GPT2_2part_Audio_Cross']:
                    with torch.no_grad():
                        quants_pred = self.model_vqvae.module.encode(body_seqs)
