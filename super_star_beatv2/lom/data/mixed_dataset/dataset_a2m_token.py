...
class Audio2MotionDatasetToken(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.selected_file.empty:
            logger.warning(f"{split} is empty for speaker {args.training_speakers}, use train set 0-8 instead")
            self.selected_file = split_rule.loc[(split_rule['type'] == 'train') & (split_rule['id'].str.split("_").str[0].astype(int).isin(self.training_speakers))]
        ...
        # preloaded_dir = pjoin( data_root , cache_path , split + f"/{pose_rep}_cache")
        preloaded_dir = pjoin( args.cache_path , split + f"/{args.pose_rep}_cache")

        if self.args.beat_align:
            if not os.path.exists(pjoin(data_root, f"weights/mean_vel_{args.pose_rep}.npy")):
                self.calculate_mean_velocity(pjoin(data_root, f"weights/mean_vel_{args.pose_rep}.npy"))
            self.avg_vel = np.load(pjoin(data_root, f"weights/mean_vel_{args.pose_rep}.npy"))
    ...
    def calculate_mean_velocity(self, save_path):
        ...
        # dir_p = self.data_dir + self.pose_rep + "/"
        dir_p = pjoin(self.data_dir, self.pose_rep )
        all_list = []
    ...
    def __getitem__(self, idx):
        ...
        with self.lmdb_env.begin(write=False) as txn:
            ...
            tar_pose, in_audio, in_facial, in_shape, in_word, emo, sem, vid, trans, id_pose = sample
            #print(in_shape)
            ...
            return {"pose":tar_pose, "audio":in_audio, "facial":in_facial, "beta": in_shape, "word":in_word, "id":vid, "emo":emo, "sem":sem, "trans":trans, "seq_name":id_pose}
