...
class MixedDatasetP2P(data.Dataset):
    def __init__(
        self,
        dataset_configs,
        split,
        ...
        **kwargs,
        ...
        for config in dataset_configs:
            ...
            elif dataset_name == "BEAT2_Mixed":
                self._load_beat2_mixed(config)
                self.data_dict.update(self.data_dict_beat2_mixed)
                self.metadata.extend(self.metadata_beat2_mixed)
                print(f"Loaded BEAT2_Mixed dataset with {len(self.metadata_beat2_mixed)} samples")
            else:
    ...
    def _load_beat2_mixed(self, config):
        """
        Load the BEAT2 dataset based on the configuration.
        """
        # Get cache path for this specific dataset
        ...
        # motion_dir_beat2 = pjoin(data_root, code_path)
        audio_dir_beat2 = pjoin(data_root, code_path_audio)
        upper_dir_beat2 = pjoin(data_root, 'TOKENS_AGENT_25/upper')
        hand_dir_beat2 = pjoin(data_root, 'TOKENS_AGENT_25/hand')
        face_dir_beat2 = pjoin(data_root, 'TOKENS_AGENT_25/face')
        lower_dir_beat2 = pjoin(data_root, 'TOKENS_AGENT_25_H3D')
        ...
        self.metadata_beat2_mixed = []
        self.data_dict_beat2_mixed = {}
        ...
        for i, name in enumerator:
            ...
            try:
                # Load motion tokens
                m_token_face = np.load(pjoin(face_dir_beat2, f'{name}.npy'))
                m_token_hands = np.load(pjoin(hand_dir_beat2, f'{name}.npy'))
                m_token_lower = np.load(pjoin(lower_dir_beat2, f'{name}.npy'))
                m_token_upper = np.load(pjoin(upper_dir_beat2, f'{name}.npy'))

                if m_token_lower.shape[1] != m_token_upper.shape[1]:
                    print(f"{name} has different length of lower and upper tokens: {m_token_lower.shape[1]} != {m_token_upper.shape[1]}")
                    min_length = min(m_token_lower.shape[1], m_token_upper.shape[1])
                    m_token_lower = m_token_lower[:, :min_length]
                    m_token_upper = m_token_upper[:, :min_length]
                ...
                if self.vary_length == True:
                    ...
                    for text, start_time, end_time in paragraphs:
                        ...
                        for tasks in self.tasks_beat2:
                            new_name = 'beat2_' + name + '_' + str(clip_index) + '_' + tasks['class']
                            self.data_dict_beat2_mixed[new_name] = {
                                'face_token': sample_face,
                                'hand_token': sample_hand,
                                'lower_token': sample_lower,
                                'upper_token': sample_upper,
                            ...
                            self.metadata_beat2_mixed.append(new_name)
                        clip_index += 1
                else:
                    # Compute the start and end times for clipping
                    round_seconds_skeleton = motion_length // self.motion_token_fps
        ...
        # Save to cache after processing
        self._save_to_cache(cache_path, self.data_dict_beat2_mixed, self.metadata_beat2_mixed, "BEAT2_Mixed")
    ...
    def _load_amass_mixed(self, config):
        ...
        # Iterate through the files to load data
        enumerator = enumerate(track(id_list, f"Loading AMASS {self.split}"))
        self.metadata_amass_mixed = []
        self.data_dict_amass_mixed = {}

        # Fast loading
        for i, name in enumerator:
            if len(self.metadata_amass_mixed) > self.maxdata:
                break
            try:
                # Load motion tokens
                upper_token_list = np.load(pjoin(upper_dir_amass, f'{name}.npy'))
                lower_token_list = np.load(pjoin(lower_dir_amass, f'{name}.npy'))
                if lower_token_list.shape[1] != upper_token_list.shape[1]:
                    print(f"{name} has different length of lower and upper tokens: {lower_token_list.shape[1]} != {upper_token_list.shape[1]}")
                    min_length = min(lower_token_list.shape[1], upper_token_list.shape[1])
                    lower_token_list = lower_token_list[:, :min_length]
                    upper_token_list = upper_token_list[:, :min_length]

                face_token_list = np.zeros_like(upper_token_list)
                hand_token_list = np.zeros_like(upper_token_list)

                for tasks in self.tasks_amass:
                    save_name = 'amass_' + name + '_' + tasks['class']
                    self.data_dict_amass_mixed[save_name] = {
                        'face_token': face_token_list,
                        'hand_token': hand_token_list,
                        'lower_token': lower_token_list,
                        'upper_token': upper_token_list,
                    ...
                    self.metadata_amass_mixed.append(save_name)
            except:
                pass


        # Save to cache after processing
        self._save_to_cache(cache_path, self.data_dict_amass_mixed, self.metadata_amass_mixed, "AMASS_Mixed")
